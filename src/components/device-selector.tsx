...
export function DeviceSelector({ className }: DeviceSelectorProps) {
  ...
  const { Icon: SelectedIcon, colorClass: selectedColorClass, label: selectedLabel } = selectedDevice
    ? getDeviceDetails(selectedDevice)
    : { Icon: Smartphone, colorClass: COLORS.sidebar, label: "" };
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <div className={cn(
            ...
            <div className={cn(
              ...
              "bg-gradient-to-br",
              selectedColorClass, // Dynamic background gradient
              ...
              {/* Dynamic Icon */}
              <SelectedIcon className="h-5 w-5 relative z-10 text-black drop-shadow-sm" />
        ...
        <DropdownMenuContent align="start" className="w-[260px] p-1.5" side="bottom" sideOffset={8}>
          ...
          {devices.length === 0 ? (
            ...
            devices.map((device) => {
              const { Icon: ListIcon, colorClass: listColorClass, label: listLabel } = getDeviceDetails(device);
              return (
                <DropdownMenuItem
                  key={device.serial}
